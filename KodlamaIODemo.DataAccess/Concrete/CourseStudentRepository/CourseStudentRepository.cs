using KodlamaIODemo.DataAccess.Abstract.CourseStudentRepository;
using KodlamaIODemo.DataAccess.Abstract.GeneralRepository;

namespace KodlamaIODemo.DataAccess.Concrete.StudentCourseRepository
{
    public class CourseStudentRepository<T> : ICourseStudentRepository<T>
    where T : class, new()
    {
        private IAdd<T> _add;
        ...
        public CourseStudentRepository(IAdd<T> add, IGetAll<T> getAll, IGetById<T> getById, IRemove<T> remove, IUpdate<T> update)
        {
            _add = add;
            _getAll = getAll;
            _getById = getById;
