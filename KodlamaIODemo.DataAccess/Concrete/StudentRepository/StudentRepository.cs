...
namespace KodlamaIODemo.DataAccess.Concrete.Student
{
    public class StudentRepository<T> : IAdd<T>
    where T : class, new()
    {
        private IAdd<T> _add;
        ...
        public StudentRepository(IAdd<T> add, IGetAll<T> getAll, IGetById<T> getById, IRemove<T> remove, IUpdate<T> update)
        {
            _add = add;
            _getAll = getAll;
            _getById = getById;
