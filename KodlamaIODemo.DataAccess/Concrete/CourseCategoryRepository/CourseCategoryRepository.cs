...
namespace KodlamaIODemo.DataAccess.Concrete.CourseCategoryRepository
{
    public class CourseCategoryRepository<T> : ICourseCategoryRepository<T>
    where T : class, new()
    {
        private IAdd<T> _add;
        ...
        public CourseCategoryRepository(IAdd<T> add, IGetAll<T> getAll, IGetById<T> getById, IRemove<T> remove, IUpdate<T> update)
        {
            _add = add;
            _getAll = getAll;
            _getById = getById;
